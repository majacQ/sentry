...
class Access(abc.ABC):
    ...
    organization_member: Optional[OrganizationMember] = None

    @property
    def role(self) -> Optional[str]:
        return self.organization_member.role if self.organization_member else None

    @cached_property
    def teams(self) -> FrozenSet[Team]:
        """Return the set of teams in which the user has actual membership."""

        if self.organization_member is None:
            return frozenset()
        return frozenset(self.organization_member.get_teams())
...
class OrganizationMemberAccess(Access):
    ...
    def has_team_access(self, team: Team) -> bool:
        ...
        if (
            ...
            and self.organization_member.organization.id == team.organization_id
        ):
            return True
    ...
    def has_project_access(self, project: Project) -> bool:
        ...
        if (
            self.has_global_access
            and self.organization_member.organization.id == project.organization_id
        ):
            return True
        return project in self.projects
